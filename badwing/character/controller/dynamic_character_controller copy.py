...
class DynamicCharacterController(CharacterController):
    ...
    def update(self, delta_time: float):
        ...
        match avatar.motion_state:
            case MotionState.GROUNDED:
                # Ground movement is now handled continuously below
                pass 
        ...
        # This runs every frame. If we are grounded, update feet 
        # based on whatever keys are CURRENTLY held.
        if avatar.motion_state == MotionState.GROUNDED:
            self._apply_ground_movement()

        # 4. Cleanup Sensors
        self.feet_touching = self._foot_handler.touching()
    ...
    def _apply_ground_movement(self):
        ...
        # Apply the friction conveyor belt
        self.foot_l.surface_velocity = (-target_vx, 0)
        self.foot_r.surface_velocity = (-target_vx, 0)
    ...
    def process_keychange(self):
        ...
        Continuous movement (Walking) is now handled in update().
        """
        avatar = self.avatar
        body = avatar.body
        vx, vy = body.velocity
        ...
        match self.avatar.motion_state:
            case MotionState.GROUNDED:
                if self.up_pressed:
                    if self.check_ladder():
                        ...
                        # Apply jump impulse immediately on transition
                        body.velocity = (vx, 0)
                        body.apply_impulse_at_local_point((0, JUMP_IMPULSE))
            ...
            case MotionState.CLIMBING:
                # Ladder movement logic can stay here or move to update 
                # depending on if you want it to stop when keys are released.
                # For now, we leave it as you had it, though moving to update is safer.
                dx = dy = 0
                if self.up_pressed:
                    dy = PLAYER_MOVEMENT_SPEED
                elif self.down_pressed:
